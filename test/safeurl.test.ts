...
import { isSafeUrl, isPrivateIPv4, isPrivateIPv6 } from '../lib/safeurl.js';
...
test('isSafeUrl — blocked hostnames', async (t) => {
    ...
    await t.test('IPv6 loopback ::1 is blocked', async () => {
        ...
        assert.strictEqual(r.safe, false);
        // ::1/128 is caught by the @microsoft/antissrf CIDR block list
...
test('isSafeUrl — RFC 1918 / loopback / special ranges blocked', async (t) => {
    ...
    for (const [url, label] of blocked) {
        await t.test(`${label} is blocked`, async () => {
            ...
            assert.ok(r.reason?.includes('blocked IP'), `expected IP blocked reason for ${label}`);
        });
    }
});
...
test('isPrivateIPv6 — link-local fe80::/10', async (t) => {
    ...
    await t.test('fec0:: is private (deprecated site-local in @microsoft/antissrf)', () =>
        assert.strictEqual(isPrivateIPv6('fec0::'), true));
});
...
test('isPrivateIPv6 — public addresses', async (t) => {
    await t.test('2606:4700:4700::1111 is NOT private (Cloudflare DNS)', () => {
        assert.strictEqual(isPrivateIPv6('2606:4700:4700::1111'), false);
    });
    await t.test('2001:4860:4860::8888 is NOT private (Google DNS)', () => {
...
test('isPrivateIPv4 — public IPs', async (t) => {
    const publicIPs = ['8.8.8.8', '1.1.1.1', '93.184.216.34', '151.101.1.140'];
    for (const ip of publicIPs) {
        await t.test(`${ip} is NOT private`, () => {
            assert.strictEqual(isPrivateIPv4(ip), false);
        });
    }
    // 0.0.0.0/8 and 240.0.0.0/4 are included in @microsoft/antissrf recommendedLatest
    await t.test('0.0.0.0 is private (reserved range)', () => {
        assert.strictEqual(isPrivateIPv4('0.0.0.0'), true);
    });
    await t.test('255.255.255.255 is private (reserved/multicast range)', () => {
